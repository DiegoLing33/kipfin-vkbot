...
import me.ling.kipfin.vkbot.activities.timetable.components.TimetableDayComponent;
import me.ling.kipfin.vkbot.activities.timetable.components.ClassroomComponent;
import me.ling.kipfin.vkbot.activities.timetable.components.ExtendedSubjectComponent;
import me.ling.kipfin.vkbot.activities.timetable.components.TimetableHeaderComponent;
import me.ling.kipfin.vkbot.activities.timetable.components.WithTimeComponent;
...
public class TimetableNowModel {
    ...
    public static String getTextOrNull(@NotNull String state, LocalTime time, TimetableMaster master) throws NoSubjectsException {
        ...
        if (!analyzer.isStarted(time)) return VKBTAnswer.ITS_EARLY.random(isTeacher);
        if (analyzer.isEnded(time)) return VKBTAnswer.ITS_LATE.random(isTeacher);
        return null;
    }
