...
public class TimetableDayModel extends MessageModel {
    ...
    public String getTextOrNull() {
        try {
            ...
            if (firstIndex > 0)
                return new VKBTAnswer(String.format("%dD_SUB", firstIndex + 1)).random(this.isTeacher());
            return null;
        } catch (NoSubjectsException ex) {
            return VKBTAnswer.NO_SUBJECTS.random(isTeacher());
        } catch (NoTimetableOnDateException ex) {
            return VKBTAnswer.NO_TIMETABLE_AT_DATE.random(isTeacher());
        }
    }
