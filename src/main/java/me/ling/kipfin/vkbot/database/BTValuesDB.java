...
public class BTValuesDB extends EntityDB<BTValue, DatabaseEntityNotFoundException> {

    /**
     * Общая база данных
     */
    public static final BTValuesDB shared = new BTValuesDB();

    @Override
    public Map<Integer, BTValue> getAll() throws SQLException {
        return SQLObjectMapper.selectAllAndMap(BTValue.class, "bot_values", "id");
    }
     ...
     */
    @NotNull
    public BTValue getValue(String type, String key) throws DatabaseEntityNotFoundException {
        for (BTValue botValue : this.getCache().values())
